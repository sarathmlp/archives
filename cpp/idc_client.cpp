 ...
 ****************************************************************************
 *
 * ID: $Id: idc_client.cpp,v 3.23.54.3.2.3 2013/11/27 07:02:40 jvikram Exp $
 *
 * LOG: $Log: idc_client.cpp,v $
 * LOG: Revision 3.23.54.3.2.3  2013/11/27 07:02:40  jvikram
 * LOG: Cisco enhancements related to returning back message to application in case of failure, compiilation fixes.
 * LOG:
 * LOG: Revision 3.23.54.3.2.2  2013/11/27 05:53:49  brajappa
 * LOG: Cisco performance enhancements
 * LOG:
 * LOG: Revision 3.23.54.3.2.1  2013/11/25 10:56:05  jvikram
 * LOG: Fixed Compilation warnings
 * LOG:
 * LOG: Revision 3.23.54.3  2013/11/12 07:23:50  jvikram
 * LOG: Fixed compilation warnings
 * LOG:
 * LOG: Revision 3.23.54.2  2013/07/09 10:19:59  jvikram
 * LOG: Fix for coverity issues
 * LOG:
 * LOG: Revision 3.23.54.1  2013/06/25 09:59:05  jvikram
 ...
 * LOG:
 ****************************************************************************/
#ident "$Id: idc_client.cpp,v 3.23.54.3.2.3 2013/11/27 07:02:40 jvikram Exp $"

#include <diameterx/base/AccountingRequest.h> 
#include <sys/time.h>
#include <netdb.h>
#include <sys/types.h>
...
ProcessRegAckAndStatus(ITS_EVENT evt)
{
    ...
    ITS_UINT port  = 0;
    ITS_CHAR ipAddr[IDSCL_IP_ADDR_LEN + 1];
    ...
    memset(ipAddr, 0, IDSCL_IP_ADDR_LEN);
    ...
    else
    {
        ...
        strcpy(ipAddr, msg.u.servInfo.ipAddr);
    }
 ...
 ****************************************************************************/
IDSCL_DLLAPI ITS_INT
IDSCL_Send(ITS_HANDLE handle, dia::codec::BaseMessage& mesg, ITS_UINT sId, 
           ITS_UINT appId, IDS_BOOLEAN newRequest)
{
    ...
    try
    {
       const dia::codec::MessageBuilderInterface *msgBuilder = 
            static_cast<const dia::codec::MessageDescriptor*>(
                mesg.GetContainerDescriptor())->GetBuilder();
       msgBuilder->BuildTo(&mesg, enBuf, (unsigned&)enLen);
       ret = ITS_SUCCESS;
    }
    catch (dia::codec::NoSuchAttributeException& e)
    {
        IDS_TRACE_DEBUG(("Exception Caught: %s , Error Cause: %s ",
                         e.name(), e.what()));
    }
    catch (dia::codec::WrongCommandTypeException& e)
    {
        IDS_TRACE_DEBUG(("Exception Caught: %s , Error Cause: %s ",
                         e.name(), e.what()));
    }
    catch (dia::codec::UnknownTypeException& e)
    {
        IDS_TRACE_DEBUG(("Exception Caught: %s , Error Cause: %s ",
                         e.name(), e.what()));
    }
    catch (dia::codec::CodecException& e)
    {
        IDS_TRACE_DEBUG(("Exception Caught: %s , Error Cause: %s ",
                         e.name(), e.what()));
    }
    ...
    {
        diameterx::base::AccountingRequest &acr = 
                                     (diameterx::base::AccountingRequest &)mesg;
    ...
    while(IDS_Connection.primaryInfo.thrInfo.isRunning)
    {
        ...
        if (ret == ITS_SUCCESS)
        {
            ...
            if (evt.src == ITS_APP_SRC)
            {
                ...
                else if (evt.data[sizeof(ITS_UINT)] == GET_PEER_STATUS_ACK_MSG)
                {
                    IDS_TRACE_DEBUG(("BThreadFunction: PEER_STATUS_ACK_MSG"));
...
IDSCL_Initialize(const char *host, ITS_INT port, IDS_TRANS_TYPE tType)
{
    ...
    if ((IDS_Connection.primaryInfo.isInitialized != ITS_TRUE) &&
        (strcmp(IDS_Connection.primaryInfo.ipAddr, "\0") == 0 ||
         (strcmp(IDS_Connection.primaryInfo.ipAddr, host) == 0 && 
          IDS_Connection.primaryInfo.port == (ITS_UINT)port) ) )
    {
    ...
    else
    {
        ...
        if (IDS_Connection.primaryInfo.port == (ITS_UINT)port
            && strcmp(IDS_Connection.primaryInfo.ipAddr, host) == 0)
        {
                     ...
                     IDS_CHAR* sessIdString)
{
    ...
    if(orgnHost.find('.') == std::string::npos)
    {
...
IDSCL_DLLAPI ITS_INT
IDSCL_GetPeerStatusMsg(ITS_HANDLE handle, IDS_MESSAGE *msg)
{
    ITS_EVENT evt;
    ...
    IDS_TRACE_DEBUG(("IDSCL_GetPeerStatusMsg"));                                                                        
