 ...
 * LOG: Revision 3.1.76.1  2013/07/09 12:26:49  jvikram
 * LOG: Call flow updated for load
 * LOG:
 ...
 * LOG: 
 *
 * ID: $Id: sample_gx.cpp,v 3.1.76.1 2013/07/09 12:26:49 jvikram Exp $
 ****************************************************************************/
...
void *findAndReleaseSession(void *)
{
    ...
    while (1)
    {
        ...
        for (it = sessQueue.begin(); it != sessQueue.end(); ) 
        {
            if (((curr_time - (*it).t_Stamp) * 1000) > INTERVAL_MS)
            {
                if (!(*it).sessionId.empty())
                {
                    ReleaseSession((*it).sessionId, (*it).sIndx);
                }
                it = sessQueue.erase(it);
            }
            else
            {
                it++;
                    ...
                    else
                    {
                        ...
                        switch(ccAns.getCCRequestType().value())
                        {
                            case CCRequestType::INITIAL_REQUEST:
                                if (appMode != UNIT_TEST)
                                {
                                    SendGxCCRequest(GX_UPDATE, impl);
                                }
                                break;
                            case CCRequestType::UPDATE_REQUEST:
                                if (appMode != UNIT_TEST)
                                {
                                    SendGxCCRequest(GX_TERMINATE,
                                            impl);
                                }
                                break;
...
int main(int argc, const char** argv)
{
    ...
    pthread_t tid;
    pthread_create(&tid, NULL, &findAndReleaseSession, NULL);
    AppMain(argc,argv);
